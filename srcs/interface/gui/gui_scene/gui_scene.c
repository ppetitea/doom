...
t_gui *build_gui_image(char *name, t_vec2i size, t_pos2i pos, char *img_src)
{
	...
	if (!(image->background = image_find(list, img_src)) && del_gui(&image->node))
							  ...
							  t_gui_interface *interface)
{
	...
	if (list == NULL || list->node.childs == NULL)
		return (console(WARNING, __func__, __LINE__, "NULL pointer").warn);
	size = ft_vec2i(parent->size.x, 50);
	pos = ft_vec2i(0, 0);
	curr = list->node.childs;
	while ((curr = curr->next) != list->node.childs)
	{
		item = (t_data *)curr;
		if ((child = build_gui_checkbox(item->key, pos, size, interface)))
		{
							   ...
							   t_gui_interface *interface)
{
	...
	if (!(checkbox_list = build_gui_scrollview(list->key, pos, size, interface)))
		return (console(FATAL, __func__, __LINE__, "gui_block fail").null);
