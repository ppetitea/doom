...
/*   Updated: 2020/05/09 23:38:14 by ppetitea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_gui *build_radio1(t_gui_interface *interface, t_gui *home)
{
	...
	if ((radio_list = data_array_s2("exemple", "radio2", "radio3")))
		node_add_child(&radio->radio->node, &radio_list->node);
	radio->display = TRUE;
...
t_gui *build_radio2(t_gui_interface *interface, t_gui *home)
{
	...
	if (!(radio = build_gui_radio("radio2", pos, size, interface)))
		return (console(FATAL, __func__, __LINE__, "build_radio fail").null);
	if ((radio_list = data_array_s2("exemple", "radio1", "radio3")))
		node_add_child(&radio->radio->node, &radio_list->node);
	radio->display = TRUE;
	node_add_child(&home->node, &radio->node);
	(void)interface;
...
t_gui *build_radio3(t_gui_interface *interface, t_gui *home)
{
	...
	if (!(radio = build_gui_radio("radio3", pos, size, interface)))
		return (console(FATAL, __func__, __LINE__, "build_radio fail").null);
	if ((radio_list = data_array_s2("exemple", "radio1", "radio2")))
		node_add_child(&radio->radio->node, &radio_list->node);
	radio->display = TRUE;
	node_add_child(&home->node, &radio->node);
	(void)interface;
...
t_gui	*build_fruit_list(t_gui_interface *interface, t_gui *home)
{
	...
	if (!(data = data_array_s3("Liste de fruits", "banana", "apple", "orange")))
		return (console(FATAL, __func__, __LINE__, "data_array fail").null);
	data_add_3child_string(data, "strawberry", "pinapple", "riceball");
...
t_gui	*build_fruit_radio_list(t_gui_interface *interface, t_gui *home)
{
	t_gui	*list;
	t_data	*data;
	t_pos2i	pos;
	...
	if (!(data = data_array_s3("Liste de fruits", "banana", "apple", "orange")))
		return (console(FATAL, __func__, __LINE__, "data_array fail").null);
	data_add_3child_string(data, "strawberry", "pinapple", "riceball");
	if (!(list = build_gui_radio_list(data, pos, size, interface)))
		return (console(FATAL, __func__, __LINE__, "build_checklist fail").null);
	node_foreach_next(&list->node, set_gui_to_display);
	list->bg_color = ft_bgra(50, 150, 100, 255);
	list->display = TRUE;
...
t_gui *build_home_scene(t_gui_interface *interface, t_vec2i screen_size)
{
	...
	build_fruit_radio_list(interface, home);
